...
import { LuSettings, LuDownload, LuCopy, LuCheck, LuPlus, LuTrash2, LuFileCode2, LuFileCode, LuCircleCheck, LuTerminal, LuRotateCcw, LuSparkles, LuCommand } from 'react-icons/lu';
...
export default function ConfigStudio({}: ConfigStudioProps) {
  ...
  const buildConfigObject = () => {
    ...
    if (envs.length > 0) {
      config.env = envs.reduce((acc, env) => {
  ...
  }, [platform, appName, port, healthCheck, disks, cronExpression, cronCommand, envs, isManualEdit]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 space-y-4">
                ...
                {platforms.map((p) => (
                  <button
                    ...
                    className={cn(
                      ...
                      platform === p.id
                        ? 'bg-gradient-to-r from-teal-500 to-primary text-white border-transparent shadow-lg shadow-primary/20 elevation-2'
            ...
            <Card className="p-4 space-y-3.5 elevation-1">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] text-muted-foreground block">مسیر بررسی سلامت (Health Check Path)</label>
                <Input
                  type="text"
                  value={healthCheck}
